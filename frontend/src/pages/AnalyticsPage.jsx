...
import { HiOutlineArrowUpTray, HiOutlineDocumentText, HiOutlineSparkles, HiOutlineTrash, HiOutlineEye } from 'react-icons/hi2';
...
const AnalyticsPage = () => {
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            <div>
                <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Data Analytics</h1>
                <p className="text-slate-500 dark:text-slate-400 mt-1">Upload CSV files and get AI-powered insights</p>
            </div>
                ...
                {file ? (
                    <div className="flex items-center justify-center gap-4">
                        ...
                        <button
                            ...
                            className="flex items-center gap-2 px-5 py-2.5 text-sm font-medium text-white rounded-xl gradient-primary hover:opacity-90 transition-all disabled:opacity-50"
                        >
                            {isAnalyzing ? (
                                <>
                    ...
                    <div>
                        ...
                        <button
                            ...
                            className="mt-3 px-5 py-2 text-sm font-medium text-primary-600 bg-primary-50 dark:bg-primary-900/30 rounded-xl hover:bg-primary-100 transition-colors"
                        >
                            Browse Files
                        </button>
                    </div>
            ...
            {/* AI Insights */}
            {insights && (
                <div className="bg-white dark:bg-slate-800/50 rounded-2xl p-6 border border-slate-200 dark:border-slate-800 animate-slide-in-up">
                    ...
                    <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed mb-4">{insights.summary}</p>
                    <h4 className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">Recommendations:</h4>
                    <ul className="space-y-2">
                        {insights.recommendations.map((rec, i) => (
                            <li key={i} className="flex items-start gap-2 text-sm text-slate-600 dark:text-slate-400">
                                <span className="w-5 h-5 rounded-full bg-primary-100 dark:bg-primary-900/30 text-primary-600 flex items-center justify-center shrink-0 text-xs font-medium mt-0.5">
                                    {i + 1}
                                </span>
                                {rec}
                            </li>
            ...
            <div className="grid lg:grid-cols-2 gap-6">
                <div className="bg-white dark:bg-slate-800/50 rounded-2xl p-6 border border-slate-200 dark:border-slate-800">
                    <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-4">Performance Trends</h3>
                    <ResponsiveContainer width="100%" height={300}>
                        <LineChart data={demoChartData}>
                            ...
                            <Line type="monotone" dataKey="revenue" stroke="#22d3ee" strokeWidth={2} dot={{ fill: '#22d3ee' }} />
                        </LineChart>
                    </ResponsiveContainer>
                </div>
